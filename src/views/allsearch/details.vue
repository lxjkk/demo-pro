  ...
  <div>
    <div v-if="agreement && isUser" style="padding-top: 9px">
      ...
      <div class="l-flex-justcenter agreement">
        <escape-component :image="2" title="您目前还没有开启全文搜索服务" content="ElasticSearch分布式搜索引擎，自动分词、关联搜索，可以有效提升搜索效率和精准度。"
          escape="全文搜索服务协议" userDesc="本服务支持试用，开通后提供3000次免费调用，如有额外需要请联系客服" @grant="grant" @escape-privacy="escapePrivacy"
    ...
    <div class="main" v-else>
      ...
      <div class="tail-info l-container">
        <a-tabs default-active-key="1">
          ...
          <a-tab-pane key="4" tab="内容库" v-if="isAdmin">
            内容库
          </a-tab-pane>
    ...
    <progress-bar :visible.sync="visible" @anew-up="anewUp" :data='{status: 1,percent:100}' />
  </div>
</template>
    ...
    grant (e) {
      if (e) {
        ...
        this.$confirm({
          centered: true,
          title: '是否立即上传数据',
          content: '首次上传可能需要较长时间，应用将进入维护状态，届时用户将无法使用各类功能',
          cancelText: '稍后上传',
          okText: '立即上传',
          icon: 'info-circle',
          onOk () {
